...
import {
  ...
  MessageCircle, DollarSign, Settings, LogOut, X, PlusCircle, Bot,
} from 'lucide-react'
...
const navItems = [
  ...
  { href: '/agents',         label: 'Agente IA',       icon: Bot,             exact: false },
...
export function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <aside className="flex h-full w-64 flex-col" style={{ backgroundColor: '#fff', borderRight: '1px solid var(--color-border)' }}>
      {/* Logo */}
      <div className="flex h-16 flex-shrink-0 items-center justify-between px-5"
        ...
        {onClose && (
          <button onClick={onClose} aria-label="Cerrar menú"
            className="icon-btn-hover ml-2 flex-shrink-0 rounded-lg p-1.5 lg:hidden"
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
        ...
        {tenant && (
          <Link href="/onboarding" onClick={onClose}
            className="nav-item-hover flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-150 mt-1"
            style={{ color: 'var(--color-muted)' }}>
            <PlusCircle className="h-4 w-4 flex-shrink-0" />
            Agregar tienda
          </Link>
      ...
      <div className="p-3" style={{ borderTop: '1px solid var(--color-border)' }}>
        <button
          ...
          className="nav-item-hover flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-150"
          style={{ color: 'var(--color-muted)' }}
        >
